...
// console.log(subFrom20(5, 5)); // => should log 15
  ...
  let obj = {}
    ...
    if (args.length === 2) {
      ...
      return args[0].split(' ').map((word) => {
        if (obj[word.substring(0, word.length - 1)]) {
          return `${obj[word.substring(0, word.length - 1)]}${word[word.length - 1]}`
        }
        return word
      }).join(' ')
    }
